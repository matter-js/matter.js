...
- @matter/node
    ...
    - Breaking: A server cluster placed where a device type's singleton rule forbids it now refuses construction with `DeviceTypeConformanceError`; it previously succeeded, or crashed with an untyped error for root-only behaviors
- @matter/testing
