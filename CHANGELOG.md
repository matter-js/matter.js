...
- @matter/protocol
    - Breaking: `ClientSubscriptions.lastReportStartedAtFor()` and `PeerSubscription.lastReportStartedAt` are removed. A report timestamp cannot say which session carried the report, so a reader could not tell a returning peer's live data from the traffic it flushed while shutting down. `ClientSubscriptions.onReport()` registers a listener that receives each inbound report together with its session instead
    - Enhancement: `RebootResubscribeArmer.isArmed()` reports whether a peer is still waiting for its reboot to resolve
    - Fix: After a peer reboots, a subscription report the peer flushed while shutting down no longer counts as evidence that its subscription survived, so the controller forces re-subscription once the grace window after the peer's return has passed instead of waiting out the full subscription timeout
