...
- @matter/node
    ...
    - Enhancement: A server endpoint's structure is checked against the device types it declares, at construction and as it changes afterward, and each violation is logged as a warning; `endpoint.validation.strict` (`MATTER_ENDPOINT_VALIDATION_STRICT`) refuses construction instead, and `Endpoint.Options.deviceConditions` states conditions the structure does not show
    - Breaking: A server cluster placed where a device type's singleton rule forbids it now refuses construction with `DeviceTypeConformanceError`; it previously succeeded, or crashed with an untyped error for root-only behaviors; this includes the optional `AdministratorCommissioningServer` and `PowerSourceConfigurationServer` that `BridgedNodeEndpoint` offers, which are RootNode singletons
- @matter/testing
