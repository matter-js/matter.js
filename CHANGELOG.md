...
- @matter/node
    ...
    - Fix: An OTA requestor subclass sends `NotifyUpdateApplied` with its update token and emits `VersionApplied` after the restart an update causes; the first update into this version sends neither. The update record is stored before `applyUpdate` runs in its own transaction, and if `applyUpdate` throws, the requestor forgets the update and returns to idle with reason `Failure`. `applyUpdate` is expected to delete the downloaded file once done or failed
    - Fix: `RemoteServer` declares `certificate` and `key` as PEM strings, and `EventsBehavior` validates `buffers`, which no longer accepts a `critical` allowance
