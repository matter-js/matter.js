...
- @matter/model
    - Fix: A constraint that names a bound held by another element, written `<Element>.<Field>`, now applies that bound. It previously evaluated to nothing and the value was accepted unchecked. This arms validation on `OccupancySensing.HoldTime`, `AmbientContextSensing.HoldTime` and `SoilMeasurement.SoilMoistureMeasuredValue`, and changes behavior in three places: a client write of either `HoldTime` outside the device's declared `HoldTimeLimits` now answers `CONSTRAINT_ERROR` where it previously answered `SUCCESS`; an application that sets one of these attributes outside its declared limits now raises `ConstraintError` at the assignment; and a node whose stored `HoldTime` lies outside its limits now fails to start, because `HoldTime` is non-volatile and its restored value is validated when the behavior initializes. Where a device previously accepted an out-of-range value it will now refuse it, so check that the limits a device declares admit the value it holds
