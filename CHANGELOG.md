...
- @matter/protocol
    ...
    - Fix: Group data message counters are now a single node-global counter (per Matter spec) instead of per operational key, persisted with a reserve block so they never roll back; existing per-key counters are migrated forward on first start to prevent nonce reuse
