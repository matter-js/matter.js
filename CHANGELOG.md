...
- @matter/protocol
    ...
    - Fix: BLE discoveries may run at the same time. `BleScanner` scans from the first discovery until the last one ends and never starts and stops a scan at once, and each discovery waits for its own result, so one discovery no longer stops another's scan, ends on another's timeout, or waits forever because another asked for the same devices
