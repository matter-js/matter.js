...
export function* scanDocument(docRef: HtmlReference) {
    ...
    function* scanPage(ref: HtmlReference, html: Document): Generator<HtmlReference> {
        ...
        // definitions in <dd><p>, formulas in stemblock/literalblock divs.  h7 is non-standard HTML but
        // generated by Asciidoctor for sect6 (6 levels deep) sections
        const elements = html.querySelectorAll(
            "h1, h2, h3, h4, h5, h6, h7, body > p, .paragraph > p, li > p, dd > p," +
            " .stemblock .content, .literalblock .content pre, table",
        );
                    ...
                    // Extract content from Asciidoctor admonition blocks (notes, warnings, etc.)
                    {
                        ...
                        if (admonitionBlock) {
                            ...
                            if (content) {
                                ...
                                // Create a synthetic "NOTE" marker to match the existing addDocumentation flow
                                const noteMarker = element.ownerDocument.createElement("p");
                                noteMarker.textContent = admonitionBlock.classList.contains("warning")
                                    ? "WARNING"
                                    : "NOTE";
                                currentRef.prose.push(noteMarker);
                                ...
                                if (contentPs.length > 0) {
                                    ...
                                    const contentP = element.ownerDocument.createElement("p");
                                    contentP.textContent = content.textContent;
                                    currentRef.prose.push(contentP);
                                }
                            }
                            break;
                        }
