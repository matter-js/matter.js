...
# Wrapper script for lit-icd-app
# Captures stdout/stderr to log file and handles SIGTERM

# Path to the real executable
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REAL_PROGRAM="$SCRIPT_DIR/dist/esm/IcdTestApp.js"

# Log file (hardcoded)
LOG_FILE="./test_lit_icd.log"
...
sigterm_handler() {
    ...
    # Log completion
    echo "$(date '+%Y-%m-%d %H:%M:%S') - lit-icd-app exited with code after SIGTERM: $EXIT_CODE" | tee -a "$LOG_FILE"
...
# Log start
echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting lit-icd-app with arguments: $@" | tee -a "$LOG_FILE"
...
# Log completion
echo "$(date '+%Y-%m-%d %H:%M:%S') - lit-icd-app exited with code: $EXIT_CODE" | tee -a "$LOG_FILE"
