...
async function readAndCheckLog(
    ...
    const read = await attempt(
        () => dut.node(ref).readAttribute(spec),
        () => "read answered",
    );
    const logCheck = await expectAttributePathIB(th.log, th.flavor, spec, from, LOG_TIMEOUT);
    if (!read.ok) {
        await recordAll(cx, [
            { what: "read", check: () => read.check },
            { what: "AttributePathIB log", check: () => logCheck },
        ]);
        throw new InternalError("a failed read recorded no failing check");
    }
    return { value: read.value, logCheck };
}

/**
 * A step's first check. The value checks after it are built lazily, so one that throws on a malformed
 * read ({@link asWildcardEntries}) does so once this one is recorded.
 */
function logCheckOf(logCheck: CheckRecord): RecordedCheck {
    return { what: "AttributePathIB log", check: () => logCheck };
}
...
/**
 * Checks a numeric read against the data type its step names.
 *
    ...
    .step(
        ...
        async cx => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "VendorID 0xfff1",
                    check: () => ({
                        type: "response",
                        verdict: value === 0xfff1 ? "pass" : "fail",
                        detail: `VendorID = ${value}`,
                    }),
                },
            ]);
        },
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Cluster-wildcard read result",
                    check: () => {
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Full-wildcard read result",
                    check: () => {
                        const entries = asWildcardEntries(value);
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Attribute-wildcard read result",
                    check: () => {
                        const entries = asWildcardEntries(value);
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Cluster-wildcard-across-endpoints result",
                    check: () => {
                        const entries = asWildcardEntries(value);
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Cluster+attribute-wildcard result",
                    check: () => {
                        const entries = asWildcardEntries(value);
                        const allMatch = entries.every(
                            entry =>
                                entry.cluster === DESCRIPTOR.id &&
                                entry.attribute === SERVER_LIST.id &&
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Endpoint+attribute-wildcard result",
                    check: () => {
                        const entries = asWildcardEntries(value);
                        const allMatch = entries.every(
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "Non-empty list of octet strings",
                    check: () => {
                        const pass =
                            Array.isArray(value) && value.length > 0 && value.every(cert => cert instanceof Uint8Array);
                        return {
                            type: "response",
                            verdict: pass ? "pass" : "fail",
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    what: "BasicCommissioningInfo struct",
                    check: () => {
                        const pass =
                            typeof value === "object" &&
                            value !== null &&
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            await withChecks(cx, async checks => {
                for (let i = 0; i < 3; i++) {
                    const { value, logCheck } = await readAndCheckLog(cx, ref, spec);
                    checks.push(logCheckOf(logCheck));
                    values.push(value);
                }
                ...
                checks.push({
                    what: "3 identical bool reads",
                    check: () => ({
                        type: "response",
                        verdict: allBoolean && allSame ? "pass" : "fail",
    ...
    .step(
        ...
        commissioned.withRef("dut", async (cx, ref) => {
            ...
            const checks: RecordedCheck[] = [
                logCheckOf(logCheck),
                {
                    what: "Large wildcard read (>100 attributes)",
                    check: () => {
                        const entries = asWildcardEntries(value);
            ...
            // The transfer is anchored on the request the log check matched, so a failed match leaves nothing to search from
            if (logCheck.verdict !== "fail") {
                checks.push({
                    what: "Chunked-transfer log",
                    check: () => expectChunkedTransfer(th.log, th.flavor, logCheck, LOG_TIMEOUT),
                });
            }
            ...
            await recordAll(cx, [
                logCheckOf(logCheck),
                {
                    ...
                    check: () => {
                        ...
                        return {
                            type: "response",
                            verdict: entries.length > 100 ? "pass" : "fail",
