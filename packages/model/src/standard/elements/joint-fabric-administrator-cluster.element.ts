...
export const JointFabricAdministrator = Cluster(
    ...
    Command({
        name: "IcaccsrRequest", id: 0x0, access: "A", conformance: "M", direction: "request",
        response: "IcaccsrResponse"
    }),
    Command(
        { name: "IcaccsrResponse", id: 0x1, conformance: "M", direction: "response" },
        Field({ name: "StatusCode", id: 0x0, type: "ICACCSRResponseStatusCodeEnum", conformance: "M" }),
        Field({ name: "Icaccsr", id: 0x1, type: "octstr", conformance: "StatusCode == Ok, O", constraint: "max 600" })
    ),
    Command(
        { name: "AddIcac", id: 0x2, access: "A", conformance: "M", direction: "request", response: "IcacResponse" },
        Field({ name: "IcacValue", id: 0x0, type: "octstr", conformance: "M", constraint: "max 400" })
    ),
    Command(
        { name: "IcacResponse", id: 0x3, conformance: "M", direction: "response" },
        Field({ name: "StatusCode", id: 0x0, type: "ICACResponseStatusEnum", conformance: "M" })
    ),

    Command(
        {
            name: "OpenJointCommissioningWindow", id: 0x4, access: "A", conformance: "M", direction: "request",
            response: "status"
        },
    ...
    Command({
        name: "TransferAnchorRequest", id: 0x5, access: "A", conformance: "M", direction: "request",
    ...
    Command({
        name: "TransferAnchorComplete", id: 0x7, access: "A", conformance: "M", direction: "request",
        response: "status"
    }),

    Command(
        {
            name: "AnnounceJointFabricAdministrator", id: 0x8, access: "A", conformance: "M",
    ...
    Datatype(
        { name: "ICACCSRResponseStatusCodeEnum", type: "enum8" },
        Field({ name: "Ok", id: 0x0, conformance: "M" }),
        Field({ name: "Busy", id: 0x1, conformance: "M" }),
        Field({ name: "PakeParameterError", id: 0x2, conformance: "M" }),
        Field({ name: "WindowNotOpen", id: 0x3, conformance: "M" }),
        Field({ name: "VidNotVerified", id: 0x4, conformance: "M" }),
        Field({ name: "InvalidAdministratorFabricIndex", id: 0x5, conformance: "M" })
    )
);
