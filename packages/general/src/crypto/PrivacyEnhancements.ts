...
export class PrivacyEnhancements {
    ...
    static async derivePrivacyKey(operationalKey: Bytes, crypto?: Crypto): Promise<Bytes> {
        ...
        return await cryptoImpl.createHkdfKey(operationalKey, salt, PRIVACY_KEY_INFO, CRYPTO_SYMMETRIC_KEY_LENGTH_BYTES);
    }
    ...
    static buildPrivacyNonce(sessionId: number, fullMic: Bytes): Bytes {
        ...
        if (micBytes.length !== CRYPTO_AEAD_MIC_LENGTH_BYTES) {
            throw new Error(`Privacy nonce requires a ${CRYPTO_AEAD_MIC_LENGTH_BYTES}-byte MIC, got ${micBytes.length}`);
        }
        return Bytes.concat(
