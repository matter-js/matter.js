...
/**
 * A simple storage migrator which **copies** source data to target
 * 
 * It does not remove source data
 */
export class StorageMigrator {

  public static async migrate(source: Storage, target: Storage): Promise<MigrationResult> {
  ...
  /**
   * Export result to log
   */
  public static resultToLog(result: MigrationResult) {
    ...
    if (result.skippedCount > 0 && result.skippedItems.length > 0) {
      ...
      for (const item of result.skippedItems) {
        logContent += `- ${this.buildContextKeyLog(item.contexts, item.key)}: ${item.error}\n`
      }
    }
  ...
  /**
   * Context, key pair to log
   */
  private static buildContextKeyLog(contexts: string[], key: string) {
   ...
   */
  private static async migrateContext(args: {
    source: Storage,
    target: Storage,
    contexts: string[],
    ...
    for (const key of keys) {
      try {
        ...
        logger.warn(`[migrate] Skipped '${this.buildContextKeyLog(contexts, key)}' entity: ${err}`)
      }
    }
    ...
    await MatterAggregateError.allSettled(
      promises, "[migrate] Error migrating sub-contexts!")
  }
}